...
#
# Created: Do 11. Apr 12:31:38 CEST 2024 (modified 14.09.2026)
...
library(RColorBrewer) # figure color palette
library(patchwork)  # combine and arrange plots

# data management
library(tidyverse)  # data management, piping %>%
...
library(decontam)   # remove contamination

# statistics
library(vegan)
library(car)        # Type II ANOVA, Levene test, VIF
library(effectsize) # eta squared
library(iNEXT)      # coverage-based standardization
...
# data.high       # Low throughput samples removed, defined by sample coverage SC < 0.98
# sample.species  # Samples on genus level -> final dataset for most analysis
# sample.filter   # Minor genera removed to simplify phylo tree

# Analysis pipeline:
# 00 Sample processing filtering
# 01 Composition Core
# 02 Alpha and beta diversity
# 03 Taxa abundance, endosymbionts
# 05 Phylo tree
# 06 Alluvial plot
# 07 Hym/Lep dataset, bee data import
# 08 Final figure export
...
## 00 data.comp  -------
# Build from sequencing data (or re-import from csv export file, goto line 134)
...
# data.comp <- subset_samples(data.ps, study=="Peru" | study=="Germany" )  
...
(data.bacteria = subset_taxa(data.bacteria, genus!="d:Bacteria_spc_spc_spc_spc")) # 7379
(data.bacteria = subset_taxa(data.bacteria, kingdom!="")) # 7379
...
percentage_removed_cyano_total  # 0.39%
percentage_removed_cyano_sample # 0.378%
...
cyano.percentremoved.total <- ggplot(data.bacteria.df  , aes(x=percentcyano, y=  cyanoremoved, color=host_subfamily, size = samplesums)) + 
  ...
  geom_hline(yintercept = 500, color = "red", linetype = "dashed") + geom_vline(xintercept = 2, color = "blue", linetype = "dashed") +
...
single.sample.check <- prune_samples(sample_names(data.comp) == "AW.B1.G069_S69", data.comp)  
sample_sums(single.sample.check) # list total sample sum
# Show genus names and taxa sums of top 20 ASVs in a sample
data.frame(genus = as.character(tax_table(single.sample.check)[names(sort(taxa_sums(single.sample.check), decreasing = TRUE)[1:50]), "genus"]),
  Taxa_Sum = sort(taxa_sums(single.sample.check), decreasing = TRUE)[1:50])

# Check occurrence of a specific ASV
tag_ASV <- c("ASV1620") 
...
# Check occurrence of a specific taxon 
...
# Show top ASVs from that taxon
sort(data.frame(sum = taxa_sums(check_taxa)), decreasing = FALSE)

## 00 data.fixed  -----------
# Remove low quality samples (troublemaker): PCR negative samples and samples with high cyano read counts
...
PCR.low.names <- sample_names(data.bacteria.low)[
  ...
  #metadata.bacteria.low$samplesums < 20000 &     # optional: Keep good throughput samples
  metadata.bacteria.low$host_genus != "Aglais" ] # Keep Aglais as outgroup in the dataset

# Define thresholds to remove samples with high Cyano reads
max_percent <- 2 # max 10%
...
data.fixed.prevdf <- calc_prevalence(data.fixed)

### Prevalence Plot inspect low prevalent phyla
prevalence <- ggplot(data.fixed.prevdf, aes(TotalAbundance, Prevalence / nsamples(data.fixed),color=phylum)) +
...
prevalence_cutoff <- ggplot(data.fixed.prevdf, aes(TotalAbundance, Prevalence / nsamples(data.fixed),color=phylum)) +
  geom_point(size = 2, alpha = 0.7) +
  geom_hline(yintercept = 0.05, alpha = 0.5, linetype = 2) + # set min prevalence
  geom_vline(xintercept = 100.0, alpha = 0.5, linetype = 2) + # set min abundance 
  # scale_x_log10() +  
...
min_prevalence <- (1.5*nsamples(data.fixed))/100
min_prevalence # equals about 3 samples --> set this number as min_prevalence
percent_of_samples <- (3*100)/nsamples(data.fixed)
percent_of_samples


### Low stringent filtering ASV level
# For Peru set min_prevalence = 3, min_total_abundance = 60 reads (0.001 percent) 
...
genus_percentage_removed  # 0.46% removed
...
percentage_removed_total #  1.27%
percentage_removed_sample # 1.17%
...
tail(prevfilter.removed.frame.sorted, 50)
...
single.sample.check <- prune_samples(sample_names(prevfilter.removed) == "AW.B1.G069_S69", prevfilter.removed)
sample_sums(single.sample.check) # list total sample sum
data.frame(
...
data.prevfilter.prevdf <- calc_prevalence(data.prevfilter)

prevalence2 <- ggplot(data.prevfilter.prevdf, aes(TotalAbundance, Prevalence / nsamples(data.prevfilter),color=phylum)) +
  geom_point(size = 2, alpha = 0.7) +
...
# 00 data.pruned  ----------------
# Remove positive controls (mock community and spike-in control taxa)
...
single.sample.check <- prune_samples(sample_names(spike.remove.finetuning) == "AW.B1.P003_S114", spike.remove.finetuning) # AW.B2.P80_S234 AW.B2.P82_S237
...
percentage_spike_total  # 2.001
percentage_spike_sample # 0.420
...
tail(spike.removed.frame.sorted,50)
...
keep_decontam.pos <- c( "ASV85", "ASV334" , "ASV8304",  "ASV14859" , "ASV1473" , "ASV12739" ,
                        "ASV121", "ASV194", "ASV3076", "ASV12115","ASV8279","ASV2235" , "ASV13250" )   
...
tail(decontam.removed.frame.sorted,50)
...
percentage_removed_decontam_total  # 0.179%
percentage_removed_decontam_sample # 0.159%
...
# (optional) export high filtered samples (filtersum >40%) to be pruned later
high_filtered_samples <- filterframe.df %>%
  filter(type == "sample", filtersum > 40) %>%  
  rownames()   

high_filtered_samples
...
## 00 data.high  SC Sample Coverage ----------------------
# Set threshold and remove low throughput samples e.g. < LT2000
# Better to filter by Sample Coverage (SC)

# Check sample abundance to find best cutoff 
sort(data.frame(sum = sample_sums(data.decontam)), decreasing = TRUE)

# Set cut-off LT2000
cutoff <- 2000

# define SC cutoff (can be high as these have been pre-filtered)
SC_cutoff <- 0.988 #0.988

# Get Sample Coverage (SC) with iNext package 
SC_decontam <- DataInfo(as(otu_table(data.decontam), "matrix"), datatype = "abundance")
# Make Assemblage names as row names and ad SC to dataframe
rownames(SC_decontam) <- SC_decontam$Assemblage
...
data.decontam.df$InvSimpson <- estimate_richness(data.decontam, measures=c("InvSimpson"))$InvSimpson
data.decontam.df$type  <- factor(data.decontam.df$type, levels = c("sample", "negative", "positive") )
data.decontam.df$SC <- SC_decontam[rownames(data.decontam.df), "SC"]
data.decontam.df$SCcutoff <- ifelse(data.decontam.df$SC > SC_cutoff, 'SC_keep', 'SC_remove')
head(data.decontam.df)
# Inspect abundance of potential contamination taxa
...
# SC list all samples
sample_SC_list <- data.frame(
  Sample = rownames(data.decontam.df),
  SampleSums = data.decontam.df$samplesum,
  SC = data.decontam.df$SC,
  Genus = data.decontam.df$host_genus ) %>%
arrange(desc(SC))

tail(sample_SC_list,40)
...
# Compare Sample Coverage vs read count cut-off
SC.reads.plot <- ggplot(data.decontam.df, aes(x=samplesum , y=SC, size = InvSimpson, color=SC < SC_cutoff, shape=type))  + geom_point(alpha=0.7) + # size = samplesum,
  geom_vline(xintercept = cutoff, alpha = 0.5, linetype = 2) + 
  geom_hline(yintercept = SC_cutoff, alpha = 0.5, linetype = 2, color = "red") +
  #scale_x_log10() +
  coord_cartesian(xlim = c(0, 80000)) +
  labs(title = paste("SC cutoff:", SC_cutoff, "vs LT cutoff:", cutoff), x = "read counts", y = "Sample Coverage (SC)" )


SC.Shannon.plot <- ggplot(data.decontam.df, aes(x=exp(Shannon), y=SC, size = samplesum, color=subtype, shape=SCcutoff))  + geom_point(alpha=0.7) + 
  geom_hline(yintercept = SC_cutoff, alpha = 0.5, linetype = 2) + 
  labs(title = paste("SC:", SC_cutoff), y = "Sample Coverage (SC)", x = "expShannon" )

SC.Simpson.plot <- ggplot(data.decontam.df, aes(x=InvSimpson, y=SC, size = samplesum, color=subtype, shape=SCcutoff))  + geom_point(alpha=0.7) + 
  geom_hline(yintercept = SC_cutoff, alpha = 0.5, linetype = 2) + 
  labs(title = paste("SC:", SC_cutoff), y = "Sample Coverage (SC)", x = "InvSimpson" )

SC.Simpson.reads.plot  <- ggplot(data.decontam.df, aes(x=InvSimpson , y=samplesum, size = SC, color=SC < SC_cutoff, shape=type))  + geom_point(alpha=0.7) +
  geom_hline(yintercept = cutoff, alpha = 0.5, linetype = 2)  


SC.per.genus <- ggplot(data.decontam.df, aes(x = fct_reorder(host_genus, -samplesum, .fun = median, na.rm = TRUE), y = SC, color = host_subfamily)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +
  geom_point(aes(shape=SCcutoff, size = InvSimpson), alpha = 0.7) +
  geom_hline(yintercept = SC_cutoff, alpha = 0.5, linetype = 2) + 
  #scale_color_manual(values=subfamily_colorfix) +
  theme_line2()  + theme(axis.text.x = element_text(angle = 60, hjust = 1) ) + 
  labs(x="",y="sample coverage (SC)" ) +
  annotate("text", x = 1, y = SC_cutoff,
           label = paste0("SC cutoff = ", SC_cutoff),
           hjust = 0, vjust = 1.5, size = 3  ) 

SC.histogram <- ggplot(data.decontam.df, aes(x = SC)) +
  geom_histogram(binwidth = 0.005) +
  geom_vline(xintercept = SC_cutoff, linetype = 2, color = "red") +
  xlab("Sample coverage") +
  ylab("Number of samples")

low_coverage_samples <- SC_decontam %>%
  filter(SC < SC_cutoff) %>%
  pull(Assemblage) 

# Optional: Combine with highly filtered samples 
# low_quality_samples <- union(low_coverage_samples, high_filtered_samples)
# data.high <- prune_samples(!(sample_names(data.decontam) %in% low_quality_samples), data.decontam)

# Remove low coverage samples
data.high <- prune_samples(!(sample_names(data.decontam) %in% low_coverage_samples), data.decontam)
data.low <- prune_samples((sample_names(data.decontam) %in% low_coverage_samples), data.decontam)
...
# checking sampling depth (sample sum should not correlate with Shannon) 
cor_test <- cor.test(data.high.df$Shannon, data.high.df$samplesum, method = "spearman")

# Get Sample Coverage (SC) with iNext package and ad to df
SC_data.high <- DataInfo(as(otu_table(data.high), "matrix"), datatype = "abundance")
rownames(SC_data.high) <- SC_data.high$Assemblage
data.high.df$SC <- SC_data.high[rownames(data.high.df), "SC"]
...
sumflophigh <- ggplot(data.high.df, aes(x=Shannon, y=samplesum, size = Sphingomonas, color=subtype))  + geom_point(alpha=0.7) + ggtitle(
  "data.high (SC removal)"  ) + ylim(0, 75000) + xlim(0, 5) + geom_hline(yintercept = cutoff, alpha = 0.5, linetype = 2) + ylab("read counts")
...
read.depth.per.genus <- ggplot(data.high.df, aes(x = fct_reorder(host_genus, -samplesum, .fun = median, na.rm = TRUE), y = samplesum, fill = host_subfamily)) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA) +
  geom_point(shape = 21, alpha = 0.7,size = 2) +
  theme_line2()  + theme(axis.text.x = element_text(angle = 60, hjust = 1) ) +  labs(x="",y="read depth", title ="read depth per genus")


samplesum.vs.shannon <- ggplot(data.high.df, aes(x = samplesum, y = Shannon)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  stat_regline_equation(formula = y ~ x, label.y = 1.2, aes(label = after_stat(rr.label)),size=3) +
  stat_cor(method = "spearman", label.y = 1.5, size = 3, p.accuracy = 0.001) + # Spearman
  scale_x_log10() +
  theme_classic() +
  labs(title = "Shannon diversity vs sequencing depth",
       subtitle = paste0("Spearman rho = ", round(cor_test$estimate, 3),
                         ", p = ", signif(cor_test$p.value, 3))   )

seq.depth.distribution <- ggplot(data.high.df, aes(x = samplesum)) +
...
sorted_data.high.df <- data.frame(
  samplesum = sample_sums(data.high),
  host_genus = sample_data(data.high)$host_genus,
  SC = SC_decontam[sample_names(data.high), "SC"] ) %>%
  arrange(desc(samplesum))

sorted_data.high.df

sorted_data.low.df <- data.frame(
  samplesum = sample_sums(data.low),
  host_genus = sample_data(data.low)$host_genus,
  SC = SC_decontam[sample_names(data.low), "SC"] ) %>%
  arrange(desc(samplesum))
...
data.low.comp <- data.low %>%
  ...
  facet_wrap(vars(host_genus), scales = "free") +
  coord_flip() +
  ggtitle(paste("data.low low coverage samples SC <", SC_cutoff )) +
  theme(axis.ticks.y = element_blank(), strip.text = element_text(face = "bold"))



LT.samples.SC = prune_samples(sample_sums(data.high)<10000, data.high)
LT.samples.SC # only LT samples
sample_data(LT.samples.SC)$samplesums <- sample_sums(LT.samples.SC)

LT.samples.SC.comp <- LT.samples.SC %>%
  comp_barplot(tax_level = "genus", n_taxa = 30, merge_other = F, label = "samplesums", sample_order = "bray") +
  facet_wrap(vars(host_genus), scales = "free") +  
  coord_flip() + ggtitle("data.high show sample reads up to LT 10000")

# mean samplesum by genus before SC cutoff
mean_SC_by_genus <- data.decontam.df %>%
  group_by(host_genus) %>%
  summarise(
    n = n(),
    mean_samplesum = round(mean(samplesum), 0),
    mean_SC = round(mean(SC), 3),
    .groups = "drop"  ) %>%
  arrange(desc(mean_samplesum))

as.data.frame(mean_SC_by_genus)

# mean samplesum by genus after SC cutoff
mean_SC_by_genus_after <- data.high.df %>%
  group_by(host_genus) %>%
  summarise(
    n = n(),
    mean_samplesum = round(mean(samplesum), 0),
...
as.data.frame(mean_SC_by_genus_after)


# Show sample sums to identify best cut-off
sink(file.path(out_dir, "00_data_sample_cutoff_SC.txt"))
"Sample sums above SC cutoff"
tail(sorted_data.high.df,20)
"SC_cutoff"
SC_cutoff
"Sample sums below SC cutoff"
print(sorted_data.low.df)
"samplesum before SC cutoff"
as.data.frame(mean_SC_by_genus)
"samplesum after SC cutoff"
as.data.frame(mean_SC_by_genus_after)
sink()


pdf(file.path(out_dir, "00_data_sample_cutoff_SC.pdf"), width=12, height=6)
plot.actino
plot.Brev 
plot.Bacillus
sum.shannon.genus
...
read.depth.per.genus
samplesum.vs.shannon
seq.depth.distribution
data.low.comp
LT.samples.SC.comp
dev.off()
...
sample.species.prevdf <- calc_prevalence(sample.species)

sample.species.prevalence <- ggplot(sample.species.prevdf, aes(TotalAbundance, Prevalence / nsamples(sample.ASV),color=phylum)) +
  geom_point(size = 2, alpha = 0.7) +
...
filterframe.df$SC <- SC_decontam[rownames(filterframe.df), "SC"]
filterframe.df$SC_removed <- ifelse(filterframe.df$SC > SC_cutoff, "SC_keep", "SC_remove")
...
sample_metadata <- filterframe.df[, c("chip", "sampleID", "collector", "host_order", "host_family", "host_subfamily", "host_tribe", "host_genus", "host_species", "BOLD_ID", "sex", "location", "sublocation", "country",
                                      ...
                                      "A_data.comp", "B_data.bacteria", "C_data.prevfilter", "D_data.pruned", "E_data.decontam", "SC" , "SC_removed", "is.neg", "is.pos" , "B_cyanoremoved", "C_prevremoved", "D_spillremoved", "E_decontamremoved", "B_cyanopercent", "C_prevfilterpercent", "D_prunedpercent", "E_decontampercent", "filtersum", "Accession", "BioProject")]
...
Top.genus <- names(sort(taxa_sums(sample.species.merged), decreasing=T)[1:12]) # top 12 taxa
...
sample.species.top.bar.group <- ggplot(group.melt,aes(x = fct_reorder(Sample, Abundance, .fun = sum, .desc = TRUE), y=Abundance, fill = genus))+
  ...
  labs(x="",y="Relative Abundance [%]", fill = "top 50 taxa") # scale_x_continuous(breaks=seq(0,6,1))
...
sample.order.top.bar.flip <- ggplot(sample.order.top.melt,aes(x = Sample2, y=Abundance, fill = order))+
  geom_bar(colour="black", stat="identity", linewidth=0.3) + 
  scale_fill_manual(values = order_palette)+
  theme_grid() + 
  scale_y_continuous(labels = scales::label_percent(scale = 100,  prefix = "", suffix = "" )) + 
...
sample.family.top.bar.flip <- ggplot(sample.family.top.melt,aes(x = fct_reorder(Sample, Abundance, .fun = sum, .desc = F), y=Abundance, fill = family))+
  ...
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "")) + 
  theme_grid() +  theme(legend.title=element_text(size=11)) + 
...
# 01 sample quantification 16S qPCR  ---------
table(sample_data(data.bacteria)$controltype)

# 16S in negative controls 
as(sample_data(data.bacteria), "data.frame") %>%
  filter(controltype == "TES") %>%
  group_by(controltype) %>%
  summarise(mean_log16S = mean(log16S, na.rm = TRUE))
#controltype mean_log16S
#  1 TES                3.60

as(sample_data(data.bacteria), "data.frame") %>%
  filter(controltype == "NEC") %>%
  group_by(controltype) %>%
  summarise(mean_log16S = mean(log16S, na.rm = TRUE))
#controltype mean_log16S
#  1 NEC                3.22
...
# 16S from processed dataset
sample.species

data.quant <- subset_samples(sample.species, log16S >=0 )

qPCR.sampledepth <- data.frame(
  samplesum = sample_sums(data.quant),
  host_genus = sample_data(data.quant)$host_genus,
  host_subfamily = sample_data(data.quant)$host_subfamily,
  type       = sample_data(data.quant)$type,
  PCR        = sample_data(data.quant)$PCR,
  qPCR       = sample_data(data.quant)$log16S )

# Sort the dataframe in decreasing order
qPCR.sampledepth.sorted <- qPCR.sampledepth[order(qPCR.sampledepth$samplesum ), ]

print(qPCR.sampledepth.sorted)

quant.df <- as(sample_data(data.quant), "data.frame") %>%
  rownames_to_column("Sample")

quant.df$host_subfamily_ordered <- factor(quant.df$host_subfamily, levels = subfamily_ordered)


data.quant.qPCR.host  <- ggplot(quant.df, aes(x=host_subfamily, y=log16S)) +
  ...
  geom_hline(yintercept = LOD, alpha = 0.5, linetype = 2) +
  labs(x="",y="log10 16S copy number", color = "host subfamily") +
...
qPCR_plot_host.revision  <- ggplot(quant.df, aes(x=log16S, y=host_subfamily_ordered)) +
  geom_boxplot(aes(group = host_subfamily))+
  ...
  scale_colour_manual(values=subfamily_colorfix) +
  #geom_vline(xintercept = LOD, alpha = 0.5, linetype = 2) +
  labs(y="",x="16S copy number per specimen", color ="host subfamily") +
  scale_x_continuous(breaks = 1:10, labels = scales::math_format(10^.x)  ) +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )

# mean log16S per subfamily
quant.df %>%
  group_by(host_subfamily) %>%
  summarise(
    n = sum(!is.na(log16S)),
    median_log16S = median(log16S, na.rm = TRUE),
    .groups = "drop"   )

# Overall median log16S copies
median(quant.df$log16S, na.rm = TRUE)
quantile(quant.df$log16S, probs = c(0.25,0.5, 0.75), na.rm = TRUE)
10^quantile(quant.df$log16S, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)

# 16S histogram
ggplot(quant.df, aes(x = log16S)) +
  geom_histogram(bins = 20) +
  theme_classic() +
  labs(x = expression(log[10]~"16S rRNA gene copies"), y = "Number of samples")

qPCR_plot_host.genus.sort <- ggplot(quant.df, aes(x = fct_reorder(host_genus, log16S, .fun = median, na.rm = TRUE), y=log16S)) +
  geom_boxplot(aes(group = host_genus))+
  geom_point(position = position_jitter(w = 0.1, h = 0), size=4, aes(color=host_subfamily, shape = country), alpha = 0.8) +
  theme_line2()+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  scale_colour_manual(values=subfamily_colorfix) +
  scale_y_continuous(breaks = 1:10, labels = scales::math_format(10^.x)  ) +
  labs(x="",y="16S copy number per specimen", color ="host subfamily") +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )


qPCR_plot_country <- ggplot(quant.df %>% filter(host_subfamily != "Danainae"), aes(x = fct_reorder(country, log16S, .fun = median, na.rm = TRUE), y=log16S)) +
...
lm.16S.country <- lm(log16S ~ host_subfamily + country, data = quant.df)
summary(lm.16S.country)
summary(lm.16S.country)$adj.r.squared 
Anova(lm.16S.country, type=2) # independent order
vif(lm.16S.country) # variance inflation factor should be <3  
plot(lm.16S.country,2)
hist(lm.16S.country$residuals) # extract the residuals
shapiro.test(residuals(lm.16S.country)) 
get_partial_R2(lm.16S.country)


# Individual tests for country effect
wilcox.test(log16S ~ country, data = subset(quant.df, host_subfamily == "Coliadinae"))
wilcox.test(log16S ~ country, data = subset(quant.df, host_subfamily == "Dismorphiinae"))
wilcox.test(log16S ~ country, data = subset(quant.df, host_subfamily == "Pierinae"))

## 16S host genus 
boxplot(quant.df$log16S ~ quant.df$host_genus) 
lm.16S.genus <- lm(log16S~ host_genus   , data =  quant.df)
summary(lm.16S.genus) 
Anova(lm.16S.genus, type=2) # independent order
plot(lm.16S.genus,2)
hist(lm.16S.genus$residuals) # extract the residuals
shapiro.test(residuals(lm.16S.genus)) # Run Shapiro-Wilk test on residuals 



pdf(file.path(out_dir, "01_sample_comp_quant_16S.pdf"), width=12, height=6)
data.quant.qPCR.host
qPCR_plot_host.revision
qPCR_plot_host.genus.sort
qPCR_plot_country
dev.off()
...
# Enterococcus            Asaia Commensalibacter          Pantoea      Lactococcus       Bartonella         Rahnella            Orbus 
#0.6826347        0.4850299        0.4491018        0.4191617        0.3772455        0.3353293        0.2754491        0.2634731 
#Acinetobacter      Entomomonas        Apibacter        Wolbachia      Pseudomonas       Raoultella         Bacillus     Dysgonomonas 
#0.2395210        0.2155689        0.2155689        0.2035928        0.1856287        0.1676647        0.1556886        0.1437126 
...
core.genus.rel.df <- core.genus.rel %>%
  psmelt() %>%  
  group_by(Sample, host_genus, host_family, host_subfamily,host_tribe, country,elevation,kw,location, sublocation,sex,no,weight,log16S) %>%
...
# Simplify, show only groups with a relative abundance ≥1% per sample (0.01)
sample.species.percent <- sample.species.rel # make copy
otu_table(sample.species.percent)[otu_table(sample.species.percent) < 0.01] <- 0 
# Remove groups per sample below abundance threshold
sample.species.percent <- prune_taxa(taxa_sums(sample.species.percent) > 0,
                                    sample.species.percent)

core.filter.species.rel <- prune_taxa(core.genus, sample.species.percent) # subset core taxa from abundance threshold dataset
tax_table(core.filter.species.rel)
core.filter.species.melt <- psmelt(core.filter.species.rel)

# Extract sample order for core / non-core figure
core_noncore_sample_order <- core.filter.species.melt %>%
  group_by(Sample) %>%
  summarise(total_abundance = sum(Abundance), .groups = "drop") %>%
  arrange(total_abundance) %>%
  pull(Sample)

# single sample core grid
sample.core.filter.abundance.grid5 <- ggplot(core.filter.species.melt %>%
                                      mutate(Sample = factor(Sample, levels = core_noncore_sample_order),
                                      genus = factor(genus, levels = core_genus_abundance$genus)),
                                      aes(x = Abundance , y = Sample, fill = genus) ) +
  geom_bar(colour="black", stat="identity", linewidth=0.3) + 
  scale_fill_manual(values = core_palette) +
  theme_grid() +  
  scale_x_continuous(labels = scales::label_percent(scale = 100),limits = c(0, 1)) +
  labs(x="core [%]",y="", fill="genus core") +
  facet_grid(host_subfamily ~ .,
             scales = "free_y", space = "free", switch = "y" ) +
  scale_y_discrete(labels = setNames(core.filter.species.melt$host_genus, core.filter.species.melt$Sample)) +
  theme(legend.title = element_text(size = 10),
        legend.text = element_text(size = 8),
        legend.key.size = unit(0.5, "cm"),
...
### non-core taxa from abundance threshold dataset 
noncore.filter.species <- subset_taxa(sample.species.percent, !taxa_names(sample.species.percent) %in% core.genus)
data.frame(sort(taxa_sums(noncore.filter.species ), decreasing = TRUE)[1:30])
noncore.filter.species.df <- psmelt(noncore.filter.species)

# Compute top taxa
top_noncore_samples <- noncore.filter.species.df %>%
  group_by(genus) %>%
...
noncore.filter.species.df <- noncore.filter.species.df%>%
  mutate(label = if_else(genus %in% top_noncore_samples, as.character(genus), "Other"),
...
noncore.samples_labels <- setdiff(unique(noncore.filter.species.df$label), "Other")
noncore.samplescolors <- c(setNames(noncorePalette(length(noncore.samples_labels)), noncore.samples_labels), Other = "grey70")

# use core_noncore_sample_order
noncore.filter.grid5 <- ggplot(noncore.filter.species.df %>%
                                 mutate(Sample = factor(Sample, levels = core_noncore_sample_order)),
                               aes(x = Abundance , y = Sample, fill = label)     ) +                     
  geom_bar(stat = "identity", colour = "black", linewidth = 0.3) +
  scale_fill_manual(values = noncore.samplescolors) +
  scale_x_reverse(labels = scales::label_percent(scale = 100), limits = c(1, 0)) +
  facet_grid(host_subfamily ~ .,
             scales = "free_y", space = "free" , switch = "y") +
  scale_y_discrete(labels = setNames(noncore.filter.species.df$host_genus,  noncore.filter.species.df$Sample)) +
  theme_grid() + labs(x = "non-core [%]", y = "", fill = "non-core") +
  theme(legend.title = element_text(size = 10),
        legend.text = element_text(size = 8),
        legend.key.size = unit(0.5, "cm"),
        strip.placement = "outside",
...
single.sample.noncore.abundance <- ggplot(noncore.filter.species.df, aes(x = fct_reorder(Sample, Abundance, .fun = sum, .desc = TRUE), y = Abundance, fill = label) ) +
  geom_bar(stat = "identity", colour = "black", linewidth = 0.3) +
  scale_fill_manual(values = noncore.samplescolors) +
  coord_flip() +  scale_y_reverse(labels = scales::label_percent(scale = 100), limits = c(1, 0)) +
  facet_wrap(~country * host_subfamily, scales = "free") +
  scale_x_discrete(labels = setNames(noncore.filter.species.df$host_genus,  noncore.filter.species.df$Sample)) +
  theme_grid() + theme(axis.text.y = element_text(face = "italic"),
...
noncoregenuscolors <- c(setNames(noncorePalette(length(noncore_labels)), noncore_labels), Other = "grey70")
...
#df.alpha <- estimate_richness(sample.species, measures = c("Shannon", "Observed", "InvSimpson")) %>%   rownames_to_column("Sample")
# replace estimate_richness by microbiome package
 
df.alpha <- microbiome::alpha(sample.species) %>%
  select(observed, diversity_shannon,  dominance_dbp,
...
# Quick correlations
pairs(alphaframe[, c("Shannon",  "Observed", "InvSimpson", "temperature",  "time", "log16S" , "weight")])
...
alphaframe_long <- alphaframe %>%
  pivot_longer(
    cols = c("Shannon", "Observed", "InvSimpson", "DBP"),
    names_to = "Index",
    values_to = "Value" )
...
alpha.shannon  <- ggplot(alphaframe, aes(x=host_subfamily_ordered, y=Shannon)) +
  ...
  theme_line2()+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  scale_colour_manual(values=subfamily_colorfix) +
  labs(x="",y="Shannon diversity", color = "host subfamily") +
...
alpha.shannon2  <- ggplot(alphaframe, aes(x=Shannon, y=host_subfamily_ordered)) +
  ...
  theme_line2()+ scale_y_discrete(limits = rev) + # reverse order
  scale_colour_manual(values=subfamily_colorfix) +
  labs(y="",x="Shannon diversity", color ="host subfamily") +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )
...
alpha.shannon.country  <- ggplot(alphaframe, aes(x=country, y=Shannon)) +
  ...
  theme_line2()+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  scale_colour_manual(values=subfamily_colorfix) +
  labs(x="",y="Shannon diversity") + # , subtitle="country"
  theme(plot.subtitle = element_text(hjust = 0.5))

alpha.subfamily.country  <- ggplot(alphaframe %>% filter(host_subfamily != "Danainae"), aes(x=country, y=Shannon)) +
  #ggplot(alphaframe %>%
  #group_by(host_subfamily) %>%
   #   filter(n_distinct(country) == 2) %>%
    #  ungroup(), aes(x = country, y = Shannon) ) +
...
partial_R2_sub_loc <- get_partial_R2(lm.shannon.subfam.location)

# plot variance explained barplot
barplot(
  partial_R2_sub_loc$partial_R2[partial_R2_sub_loc$term != "Residuals"],
  names.arg = partial_R2_sub_loc$term[partial_R2_sub_loc$term != "Residuals"],
  las = 2,
  col = "skyblue",
  ylab = "Partial R²",
...
eta_squared(Anova(lm.shannon.subfam.location, type = 2), partial = TRUE)
# same as partial_R2
...
# variance partitioning for lm model on Type II ANOVA
partial_R2_tribe_country <- get_partial_R2(lm.shannon.tribe.country)

# plot variance explained barplot
barplot(
  partial_R2_tribe_country$partial_R2[partial_R2_tribe_country$term != "Residuals"],
  names.arg = partial_R2_tribe_country$term[partial_R2_tribe_country$term != "Residuals"],
  las = 2,
  col = "skyblue",
  ylab = "Partial R²",
  main = "Variance partitioning" )
...
# Partial R2 or eta squared (library effectsize)
eta_squared(Anova(lm.shannon.tribe.country, type = 2), partial = TRUE)
...
sample.shannon.elevation.Peru <-
  ...
  scale_color_manual(values=subfamily_colorfix) +
  labs(x="Elevation [m]",y="Shannon diversity", fill="host subfamily",  subtitle="Peru samples" ) +
...
sample.rich.elevation.Peru <-
  ...
  scale_color_manual(values=subfamily_colorfix) +
  labs(x="Elevation [m]",y="microbial richness", fill="host subfamily") 
  coord_cartesian(ylim = c(0, 4), expand = TRUE,  default = FALSE,   clip = "on" ) 
...
sample.shannon.temp.Peru <-
    ...
    stat_regline_equation(label.y = 4.0, aes(label = after_stat(rr.label)),size=3) + stat_cor(label.y = 3.7, size=3, p.accuracy = 0.00001) +
    scale_color_manual(values=subfamily_colorfix) +
    labs(x="Temperature [°C]",y="Shannon diversity", fill="host subfamily") +
    coord_cartesian(ylim = c(0, 4), expand = TRUE,  default = FALSE,   clip = "on" ) 
...
# variance partitioning for lm model on Type II ANOVA
partial_R2_Peru_subfam_temp <- get_partial_R2(lm.peru)
...
shannon.kw.Germany.noAglais <-
  ...
  labs(x="Calendar week",y="Shannon diversity",
...
shannon.kw.Germany.noAglais2 <- ggplot(alphaframe.Germany.noAglais, aes(x = kw, y = Shannon)) +
  ...
  labs(x = "Calendar week", y = "Shannon diversity", subtitle="Germany samples", 
...
ger_predictors <- c("host_family", "host_subfamily", "host_tribe",
                     "host_genus", "location", "sublocation",
                     "kw",  "temperature", "kw + host_subfamily" )
...
lm.shannon.kw.subfam <- lm(Shannon ~ kw + host_subfamily + temperature , data=alphaframe.Germany.noAglais) 
...
# variance partitioning for lm model on Type II ANOVA
partial_R2_Ger_sub_kw <- get_partial_R2(lm.shannon.kw.subfam)

# Partial R2 or eta squared (library effectsize)
eta_squared(Anova(lm.shannon.kw.subfam, type = 2), partial = TRUE)
# Calendar week more important than host!

lm.shannon.kw.genus <- lm(Shannon ~ kw +host_genus +temperature , data=alphaframe.Germany.noAglais) 
...
lm.shannon.kw <- lm(Shannon ~ kw + host_subfamily, data=alphaframe.Germany.noAglais) 

AIC(lm.shannon.kw.subfam, lm.shannon.kw.genus,lm.shannon.kw)
...
temp.richness.noAglais2  <- ggplot(alphaframe.noAglais, aes(x = temperature, y = Observed)) +
  theme_line2() + 
  geom_point(size = 4, alpha = 0.8, aes(color=host_subfamily, shape=country)) +
  geom_smooth(method = "lm", color = "black", linewidth = 0.5, se = TRUE) +
  stat_regline_equation(label.y = 100.0, aes(label = after_stat(rr.label)),size=3) + stat_cor(label.y = 94 ,size=3, p.accuracy = 0.00001) +
  geom_point(data = alphaframe.Aglais, aes(x = temperature, y = Observed, shape = "Aglais", fill=host_subfamily), stroke = 1, size = 4) +
  scale_shape_manual(
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(
      ...
      order = 2 ) ) +
  scale_color_manual(values = subfamily_colorfix) +
  labs(x="Temperature [°C]",y="Observed richness", subtitle="Richness", color = "host subfamily") +
  #coord_cartesian(ylim = c(0, 4), expand = TRUE,  default = FALSE,   clip = "on" ) +
  guides(color = guide_legend(order = 1), shape = guide_legend(order = 2) )
...
temp.shannon.noAglais2  <- ggplot(alphaframe.noAglais, aes(x = temperature, y = Shannon)) +
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(
      ...
      order = 2 ) ) +
  scale_color_manual(values = subfamily_colorfix) +
  labs(x="Temperature [°C]",y="Shannon diversity", subtitle="Shannon", color = "host subfamily") +
  coord_cartesian(ylim = c(0, 4), expand = TRUE,  default = FALSE,   clip = "on" ) +
  guides(color = guide_legend(order = 1), shape = guide_legend(order = 2) )


temp.InvSimpson.noAglais2  <- ggplot(alphaframe.noAglais, aes(x = temperature, y = InvSimpson)) +
  theme_line2() +
  geom_point(size = 4, alpha = 0.8, aes(color=host_subfamily, shape=country)) +
  geom_smooth(method = "lm", color = "black", linewidth = 0.5, se = TRUE) +
  stat_regline_equation(label.y = 22.0, aes(label = after_stat(rr.label)),size=3) + stat_cor(label.y = 20.0 ,size=3, p.accuracy = 0.00001) +
  geom_point(data = alphaframe.Aglais, aes(x = temperature, y = InvSimpson, shape = "Aglais", fill=host_subfamily), stroke = 1, size = 4) +
  scale_shape_manual(
    values = c("Germany" = 19, "Peru" = 17, "Aglais" = 21),
    breaks = c("Germany", "Peru"),  # This controls the first shape legend
    guide = guide_legend(title = "Country", order = 1) ) +
  scale_fill_manual(
    ...
    guide = guide_legend(
      ...
      order = 2 ) ) +
  scale_color_manual(values = subfamily_colorfix) +
  labs(x="Temperature [°C]",y="Inverse Simpson diversity", subtitle="Inverse Simpson" ,  color = "host subfamily") +
  coord_cartesian(ylim = c(0, 22), expand = TRUE,  default = FALSE,   clip = "on" ) +
  guides(color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )
...
predictors_all <- c("host_family", "host_subfamily", "host_tribe",
                    ...
                    "host_subfamily + temperature" , "host_subfamily + location" , "host_subfamily + temperature +location" )
...
lm.rich.temp <- lm(Observed ~ host_subfamily  + temperature , data=alphaframe.noAglais) 
...
# variance partitioning for lm model on Type II ANOVA
partial_R2_rich.temp <- get_partial_R2(lm.rich.temp)

# eta squared
eta_squared(Anova(lm.rich.temp, type=2), partial = TRUE)



## Linear model Shannon (complete dataset from both countries host_subfamily * temperature)
...
partial_R2_temp.subfam <- get_partial_R2(lm.temp.subfam)

# Partial R2 or eta squared (library effectsize)
eta_squared(Anova(lm.temp.subfam, type = 2), partial = TRUE)

## Linear model invSimpson
#plot(alphaframe$InvSimpson ~ alphaframe$temperature) 
lm.invSimpson.temp <- lm(InvSimpson ~ host_subfamily  + temperature , data=alphaframe.noAglais) 
summary(lm.invSimpson.temp)
summary(lm.invSimpson.temp)$adj.r.squared
Anova(lm.invSimpson.temp, type=2) # independent order
vif(lm.invSimpson.temp, type = "predictor") # variance inflation factor should be <3
plot(lm.invSimpson.temp,2)
hist(lm.invSimpson.temp$residuals) # extract the residuals
shapiro.test(residuals(lm.invSimpson.temp)) # Run Shapiro-Wilk test on residuals 

# variance partitioning for lm model on Type II ANOVA
partial_R2_invSimpson.temp <- get_partial_R2(lm.invSimpson.temp)

# eta squared
eta_squared(Anova(lm.invSimpson.temp, type=2), partial = TRUE)
...
temp.observed_subfamily <- ggplot(alphaframe, aes(x = temperature, y = Observed, color=host_subfamily)) +
  theme_line2() + 
  ...
  labs(x="Temperature [°C]",y="microbial richness", title="", color="host subfamily" ) +
  scale_colour_manual(values=subfamily_colorfix) +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )

lm.temp <- lm(log16S ~ temperature , data=alphaframe) 
lm.quad.temp <- lm(log16S ~ poly(temperature, 2) , data=alphaframe) # quadratic fit 
lm.quad.temp.host <- lm(log16S ~ poly(temperature, 2) + host_subfamily , data=alphaframe) # quadratic fit 

AIC(lm.temp ,lm.quad.temp, lm.quad.temp.host )

summary(lm.quad.temp.host)
summary(lm.quad.temp.host)$adj.r.squared
Anova(lm.quad.temp.host, type=2) 
plot(lm.quad.temp.host,2)
hist(lm.quad.temp.host$residuals) # extract the residuals
shapiro.test(residuals(lm.quad.temp.host))  
partial_R2_quad.temp.host <- get_partial_R2(lm.quad.temp.host)


summary(lm.quad.temp)
plot(lm.quad.temp,2)
hist(lm.quad.temp$residuals) # extract the residuals
shapiro.test(residuals(lm.quad.temp))  

fstat <- summary(lm.quad.temp)$fstatistic
f_val <- round(fstat[1], 2)
p_val <- pf(fstat[1], fstat[2], fstat[3], lower.tail = FALSE)
temp_label_text <- paste0(
  "F == ", f_val, "~','~italic(p) ", 
  ifelse(p_val < 0.00001, "< 0.00001", paste0("= ", round(p_val, 4))) )


temp.16Scopy <- ggplot(alphaframe, aes(x = temperature, y = log16S )) +
  theme_line2() + 
  geom_point(size = 4, alpha = 0.8, aes(color=host_subfamily, shape=country)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), color = "black", linewidth = 0.5, se = TRUE) +
  stat_regline_equation(formula = y ~ poly(x, 2), label.y = 9, aes(label = after_stat(rr.label)),size=3) +
  #stat_cor(label.y = 8.5, size = 3, p.accuracy = 0.001) + 
  annotate("text", x = 7.0, y = 8.5, label = temp_label_text, size = 3, hjust = 0, parse = TRUE) +
  scale_y_continuous(breaks = 1:10, labels = scales::math_format(10^.x)  ) +
  labs(x="Temperature [°C]",y="16S copy number per specimen", title="", subtitle="All samples", color="host subfamily" ) +
  scale_colour_manual(values=subfamily_colorfix)
...
core.shannon.auto <- ggplot(alphaframe, aes(x = genus_core, y = Shannon )) +
  ...
  labs(x="genus core [%]",y="Shannon diversity", title="", color="host subfamily" ) +
  scale_colour_manual(values=subfamily_colorfix)
...
core.shannon.abund <- ggplot(alphaframe, aes(x = genus_core, y = Shannon )) +
  theme_line2() + 
  geom_point(alpha = 0.8, aes(color=host_subfamily, size=log16S, shape=country)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2),  color = "black", linewidth = 0.5, se = TRUE) +
  stat_regline_equation(formula = y ~ poly(x, 2), label.y = 0.4, aes(label = after_stat(rr.label)),size=3) +
  annotate("text", x = 0.0, y = 0.2, label = label_text, size = 3, hjust = 0, parse = TRUE) +
  scale_x_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "")) + 
  scale_size_continuous(range = c(1, 6), name = expression(log[10]~"16S") ) +
  labs(x="genus core [%]",y="Shannon diversity", title="", color="host subfamily" , size="log10 16S copies" ) +
  scale_colour_manual(values=subfamily_colorfix) +
  guides(
    color = guide_legend(order = 1,override.aes = list(size = 3)), # manual legend order
    shape = guide_legend(order = 2,override.aes = list(size = 3)) )



# final model core + 16S explain Shannon with quadratic relationship
lm.shannon.core.log16S <- lm(Shannon ~ poly(genus_core, 2) + poly(log16S, 2), data = alphaframe)
summary(lm.shannon.core.log16S)
Anova(lm.shannon.core.log16S, type=2) # both retain independent nonlinear associations with Shannon diversity when included in the same model
plot(lm.shannon.core.log16S,2)
hist(lm.shannon.core.log16S$residuals) # extract the residuals
shapiro.test(residuals(lm.shannon.core.log16S))  
vif(lm.shannon.core.log16S)

# variance partitioning for lm model on Type II ANOVA
partial_R2_shannon.core.log16S <- get_partial_R2(lm.shannon.core.log16S)


pdf(file.path(out_dir, "02_sample_div_alphaframe_temp_core.pdf"), width=12, height=6)
temp.richness.noAglais2
temp.shannon.noAglais2
temp.InvSimpson.noAglais2 
temp.observed_subfamily 
temp.16Scopy 
core.shannon.auto 
core.shannon 
core.shannon.temp 
...
"lm Richness Temperature subfamily"
summary(lm.rich.temp)$adj.r.squared
Anova(lm.rich.temp, type=2) 
partial_R2_rich.temp
cat("\n")  # blank line

"lm Shannon Temperature subfamily"
summary(lm.temp.subfam)$adj.r.squared 
Anova(lm.temp.subfam, type=2)
partial_R2_temp.subfam
cat("\n")  # blank line

"lm InvSimpson Temperature subfamily"
summary(lm.invSimpson.temp)$adj.r.squared
Anova(lm.invSimpson.temp, type=2)
partial_R2_invSimpson.temp 
cat("\n")  # blank line
...
# dimensions: 3 Stress:     0.214 # better with 3 dimensions
...
NMDS.binder.subfamily <- ggplot(nmds.binder, aes(x=NMDS1,y=NMDS2))+
...
PCoA.host.overview.tribe  <- ggplot(pcoa.binder, aes(Axis.1, Axis.2)) +
  stat_ellipse(type = "t",level = 0.9, linewidth = 0.7, alpha = 0.5, aes(group = host_tribe, color = host_tribe),show.legend = FALSE) +
  geom_point(size = 4, aes(shape = country, color = host_tribe), alpha = 0.8) +
  facet_wrap(~host_family) +
  scale_colour_manual(values = tribe_colorfix) +
  labs(x = xlab_pcoa1, y = ylab_pcoa2, color ="host tribe") +
  theme_grid() +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2)) 
...
PCoA.genus.wrap.tribe <- ggplot(pcoa.binder, aes(Axis.1, Axis.2), color = host_tribe)+
  ...
  geom_point(size=4, alpha = 0.8, aes(shape = country, color = host_tribe)) +
  facet_wrap(~host_genus_grouped) + # , ncol = 4
  scale_color_manual(values = tribe_colorfix) +
  labs(x = xlab_pcoa1, y = ylab_pcoa2) +
  guides(
    color = guide_legend(order = 1), # manual legend order
...
# sort by subfamily
betadisp.subfamily <- ggplot(betaframe, aes(x = fct_reorder(host_subfamily, beta_subfamily, .fun = median, na.rm = TRUE), y = beta_subfamily)) +
  geom_boxplot() +
...
betadisp.tribe <- ggplot(betaframe, aes(x=fct_reorder(host_tribe, beta_tribe, .fun = median, na.rm = TRUE), y=beta_tribe))+
  geom_boxplot() +
  geom_point(position = position_jitter(w = 0.1, h = 0), size=4, aes(color = host_subfamily),alpha = 0.8) +
  scale_colour_manual(values=subfamily_colorfix) +  theme_line2() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
...
beta_tribe_robust <- betadisper(genus.vegdist, group = genus.robust.df$host_tribe)
beta_subfam_robust <- betadisper(genus.vegdist, group = genus.robust.df$host_subfamily)
beta_country_robust <- betadisper(genus.vegdist, group = genus.robust.df$country)
beta_location_robust <- betadisper(genus.vegdist, group = genus.robust.df$location)

# robust tests on genus.robust
permutest(beta_genus_robust, permutations = 9999)
plot(beta_genus_robust)

permutest(beta_tribe_robust, permutations = 9999)
plot(beta_tribe_robust)

permutest(beta_subfam_robust, permutations = 9999)
plot(beta_subfam_robust)

permutest(beta_country_robust, permutations = 9999)
plot(beta_country_robust)

permutest(beta_location_robust, permutations = 9999)
plot(beta_location_robust)
...
betadisp.genus   <- ggplot(genus.robust.df  , aes(x=fct_reorder(host_genus, beta, .fun = median, na.rm = TRUE), y=beta))+
  ...
  scale_colour_manual(values=tribe_colorfix) +
  theme_line2()+ theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(subtitle="", y="distance to centroid", x='', color ="host tribe") +
  guides(
    color = guide_legend(order = 1), # manual legend order
    shape = guide_legend(order = 2) )
...
# Robustness Check against over fitting  (use data with genus with <2 samples removed)
genus.robust

table(genus.robust.df$host_genus, genus.robust.df$country)
table(genus.robust.df$host_genus, genus.robust.df$location)

set.seed(781)
# test adonis individual taxomomic ranks
adonis.fam  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_family , data=genus.robust.df,  permutations = 9999, by = "margin")
adonis.fam  # 3.5% 
#              Df SumOfSqs      R2     F Pr(>F)    
# host_family   1    2.123 0.03545 5.8434  1e-04 ***

adonis.subfam  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_subfamily , data=genus.robust.df,  permutations = 9999, by = "margin")
adonis.subfam  # 18%
#              Df SumOfSqs      R2     F Pr(>F)    
# host_subfamily   6   10.926 0.18241 5.7262  1e-04 ***

adonis.tribe  <-  adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_tribe , data=genus.robust.df,  permutations = 9999, by = "margin")
adonis.tribe # 24.7%
#              Df SumOfSqs      R2     F Pr(>F)    
# host_tribe  10   14.829 0.24755 4.9349  1e-04 ***

adonis.genus  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_genus , data=genus.robust.df,  permutations = 999, by = "margin")
adonis.genus  # 33%
#              Df SumOfSqs      R2     F Pr(>F)    
# host_genus  17   19.697 0.32882 4.121  0.001 ***

adonis.species  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_species , data=genus.robust.df,  permutations = 999, by = "margin")
adonis.species # 40%
#              Df SumOfSqs      R2     F Pr(>F)    
# host_species  28   23.898 0.39896 3.1292  0.001 ***


adonis.storage  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~   host_tribe + storage + location  , data=genus.robust.df,  permutations = 999, by = "margin")
adonis.storage
...
adonis.country  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~ country , data=genus.robust.df,  permutations = 9999, by = "margin")
adonis.country
# country    1    3.592 0.05773 10.11  1e-04 *** # alone 6%

adonis.location  <- adonis2(phyloseq::distance(genus.robust, method = "bray")~ location  , data=genus.robust.df,  permutations = 9999, by = "margin")
adonis.location
...
#### Robustness Check (filter subfamily with ≥10 samples)
host_subfamily_counts <- genus.robust.df %>%
  group_by(host_subfamily) %>%
  summarise(n_samples = n())

data.frame(host_subfamily_counts[order(host_subfamily_counts$n_samples), ])

subfamily_to_keep <- host_subfamily_counts %>%
  filter(n_samples >= 10) %>%
  pull(host_subfamily)

subfam.robust <- subset_samples(genus.robust, host_subfamily %in% subfamily_to_keep)
subfam.robust.df <- data.frame(sample_data(subfam.robust))

# sample.species.rel   - 167 samples 
# genus.robust         - 161 samples
# subfam.robust        - 147 samples

set.seed(456)
adonis.robust.full  <- adonis2(phyloseq::distance(subfam.robust, method = "bray")~ host_species +  location + temp_week + kw+  elevation   , data=subfam.robust.df,  permutations = 999)
adonis.robust.full #    
# species                 Model     24   19.202 0.35784 2.8327  0.001 ***
# +loc +temp + elev + kw  Model     29   20.799 0.38761 2.5536  0.001 ***
# Adding environmental factors to species model increases R2 only minimal by 3%! 

# Quick predictor correlation matrix
cor(subfam.robust.df [, c("temp_week", "kw", "elevation")]) # quick check predictors
cor_matrix <- cor(subfam.robust.df[, c("temp_week", "kw", "elevation")])
...
subfam.robust.df$dummy <- seq_len(nrow(subfam.robust.df)) 
lm_dummy <- lm(dummy ~  host_tribe +  location + elevation +  temp_week + kw    , data = subfam.robust.df )
#summary(lm_dummy)
...
# species + temp, kw,
# genus   + temp, kw, elev
# tribe   + temp, kw, elev, location <- use for final analysis
# subfam  + temp, kw, elev, location

# Use all predictors in multivariate model
adonis.robust.tribe  <- adonis2(phyloseq::distance(subfam.robust, method = "bray")~ host_tribe + location  +  elevation + kw  + temp_week , data=subfam.robust.df,  permutations = 999, by = "margin")
adonis.robust.tribe
# host_tribe   6    7.350 0.13697 4.1342  0.001 ***
# location     3    1.159 0.02161 1.3044  0.069 .  
# elevation    1    0.322 0.00600 1.0865  0.354    
# kw           1    0.353 0.00657 1.1899  0.254    
# temp_week    1    0.434 0.00809 1.4645  0.087 . 

adonis.robust.tribe.full  <- adonis2(phyloseq::distance(subfam.robust, method = "bray")~ host_tribe + location +  elevation + kw + temp_week , data=subfam.robust.df,  permutations = 999)
adonis.robust.tribe.full
...
marginalR2.plot <- ggplot(r2_df_clean, aes(x = term, y = R2)) +
  ...
  geom_text(aes(label = signif), 
            vjust = 0.1, # place above the bar
            size = 5) +    # adjust size
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     expand = expansion(mult = c(0, 0.1))) + # add space above bars
  labs(subtitle="", x = NULL, y = "partial R² (marginal)" ) +
  theme_line2() +
  theme(axis.text = element_text(size = 9), axis.title.y = element_text(size = 10),axis.text.x = element_text(angle = 45, hjust = 1)  )


#### Variance partitioning beta diversity
community.matrix <- as(otu_table(subfam.robust), "matrix")
if (taxa_are_rows(subfam.robust)) {
  community.matrix <- t(community.matrix) }

# Hellinger transformation
comm_hel <- decostand(community.matrix, method = "hellinger")
stopifnot(identical(rownames(comm_hel), rownames(subfam.robust.df)))
subfam.robust.df <- subfam.robust.df[rownames(comm_hel), , drop = FALSE]

# Use only predictors without colinearity (checked above vif(lm_dummy))
X_host     <- subfam.robust.df[, "host_tribe", drop = FALSE]
X_location <- subfam.robust.df[, "location", drop = FALSE]
X_temp  <- subfam.robust.df[, c("temp_week"), drop = FALSE]
X_env  <- subfam.robust.df[, c("temp_week", "kw",  "location" , "elevation"), drop = FALSE]
...
# Variance partitioning using varpart() function to decompose shared vs unique fractions visually
vp_env <- varpart(comm_hel, X_host, X_env) # Host vs Environment
vp_env # automatic warns if colinearity is detected
plot(vp_env, # gives a Venn diagram-style plot
     digits = 2,
     bg = c("skyblue", "salmon"),
...
vp_temp <- varpart(comm_hel, X_host, X_temp) # Host vs Temperature
vp_temp # automatic warns if colinearity is detected
plot(vp_temp,
     digits = 2,
     bg = c("skyblue", "salmon"),
     Xnames = c("Host", "Environment"))

# using only temperature, shows minor influence of temp
...
vp_location <- varpart(comm_hel, X_host, X_location)
vp_location # automatic warns if colinearity is detected
plot(vp_location,
     digits = 2,
     bg = c("skyblue", "salmon"),
     Xnames = c("Host", "Environment"))
...
rda_host <- rda(comm_hel ~ host_tribe + Condition(kw + temp_week + location), data = subfam.robust.df)
anova(rda_host, permutations = 999)
RsquareAdj(rda_host)

# Test environment controlling for host
rda_env <- rda(comm_hel ~ kw + temp_week + location +  Condition(host_tribe), data = subfam.robust.df)
...
## 03 taxa abundance core / ASV / endo  --------------
...
core_country_label <- core_stats_country %>%
  mutate(p_label = ifelse(p_adj < 0.001, "p < 0.001",
      sprintf("p = %.3f", p_adj)), label = paste(p_label, signif),
    p_color = ifelse(p_adj < 0.05, highest_in, "ns"))
...
core_stripchart.country.stats <- ggplot(core.melt, aes(x = country, y = Abundance)) +
  ...
  theme_grid() + theme(axis.text.x = element_text(size = 8), strip.text = element_text(face = "italic")) +
  scale_colour_manual(values = country_colorfix) +
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "")) +
  labs(x = "", y = "rel. ab. [%]", color = "country") +
  geom_text(data = core_country_label,
    aes(x = 1.5, y = 0.95, label = label,color = p_color ), show.legend = FALSE, inherit.aes = FALSE, size = 3  ) 
...
ASV.bubble.core.country <- ggplot(sample.ASV.core.top.melt, aes(sampleID,  fct_reorder(OTU, genus, .desc = TRUE) )) +
  geom_point(aes (size = Abundance, color = genus ), alpha = 0.8) + # color = factor(genus, levels = core_genus_abundance$genus)
  theme_line2() +
  facet_wrap(~genus + country, scales = "free", ncol = 4) +
  scale_color_manual(values = core_palette, name = "genus") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank() ) +
  guides(color = guide_legend(override.aes = list(size = 4))) +  # bigger dots in legend
...
combined_rel_prev.df <- data.frame(
  ...
  #Dismorphiinae = Dismorphiinae_prevalence_df$rel_prev, # no prevalence with low sample number group
...
AlluTaxaCountryreverse <- ggplot(df_sorted, aes(axis1 = Sample, axis2 = Taxa, y = Abundance, fill = Taxa)) +
  geom_alluvium(aes(fill = Taxa), width = 0.2, knot.pos = 0.3, alpha = 0.6) +  
  geom_stratum(color = "grey20", width = 0.2, alpha = 0.6)  +  # "grey40"
  geom_text(stat = "stratum", aes(label = after_stat(stratum),
                                  angle = after_stat(ifelse(after_stat(x) < 1.5, 90, 0)),  # rotate only axis1 labels
                                  hjust = after_stat(ifelse(after_stat(x) < 1.5, 0.5, 1)),
                                  x = after_stat(x + ifelse(after_stat(x) > 1.5, 0.08, 0))
                                  ),   size = 3.5 ) + # 0.5 center 1.0 left align
    scale_fill_manual(values=core_palette) + 
  scale_x_discrete(limits = c("Taxa", "Sample"), expand = c(0.2, 0.1), guide = guide_axis(n.dodge = 1)) +
  theme_heat() + 
  theme(
...
# 07 Bee dataset import ----

# Re-import full bee dataset from csv export file
re_otu_bee <- read.csv(file.path(data_dir, "bee.ASV.otu.csv"), row.names = 1)
otu_bee <- otu_table(as.matrix(re_otu_bee), taxa_are_rows = T)
re_tax_bee <- read.csv(file.path(data_dir, "bee.ASV.tax.csv"), row.names = 1)
tax_bee <- tax_table(as.matrix(re_tax_bee))
re_meta_bee <- read.csv(file.path(data_dir, "bee.ASV.metadata.csv"), row.names = 1)
meta_bee <- sample_data(re_meta_bee)

# Reconstruct phyloseq object
sample.bee.ASV <- phyloseq(otu_bee, tax_bee, meta_bee)

print(object.size(re_otu_bee), units = "auto")
print(object.size(otu_bee), units = "auto")
rm(re_otu_bee)
rm(otu_bee)

# Subset ASV level
#sample.bee.ASV.Hymenoptera <- subset_samples(sample.bee.ASV, host_order=="Hymenoptera")
sample.bee.ASV.Lepidoptera <- subset_samples(sample.bee.ASV, host_order=="Lepidoptera")

# tax glom genus level
sample.bee.species <- tax_glom(sample.bee.ASV,taxrank="genus") # tax_glom processing takes time
taxa_names(sample.bee.species) <- tax_table(sample.bee.species)[,"genus"]
sample.bee.rel <- transform_sample_counts(sample.bee.species, function(x) x/sum(x))

sample.Hymenoptera <- subset_samples(sample.bee.species, host_order=="Hymenoptera")
sample.Lepidoptera <- subset_samples(sample.bee.species, host_order=="Lepidoptera")

sample.Hymenoptera.rel <- transform_sample_counts(sample.Hymenoptera, function(x) x/sum(x))
sample.Lepidoptera.rel <- transform_sample_counts(sample.Lepidoptera, function(x) x/sum(x))

# > check taxa ----
check_taxa <- subset_taxa(sample.bee.ASV, genus == "Floricoccus") # 
sum(sample_sums(check_taxa))

sort(data.frame(sum = sample_sums(check_taxa),
                host_genus = sample_data(check_taxa)$host_genus), decreasing = T)
data.frame(
  genus = as.character(tax_table(check_taxa)[names(sort(taxa_sums(check_taxa), decreasing = F)), "genus"]),
  Taxa_Sum = sort(taxa_sums(check_taxa), decreasing = F))


# check taxa
my_taxon <- c("Proteobacteria spc")
check_taxa <- subset_taxa(sample.Hymenoptera.rel, genus == my_taxon)
tail(sort(data.frame(sum = sample_sums(check_taxa)*100,
                     host_genus = sample_data(check_taxa)$host_genus), decreasing = FALSE),20)




# Lep Hym groups n size
table(sample_data(sample.bee.species)$host_order)
# Hymenoptera Lepidoptera 
# 874         167 

table(sample_data(sample.Lepidoptera)$country)

table(sample_data(sample.Hymenoptera)$host_subfamily)
#  Apinae Halictinae 
#  670        204 

table(sample_data(sample.Hymenoptera)$host_tribe)
#   Apini    Bombini Euglossini Halictinae Meliponini 
#   69         45        140        204        416 

# check bee genera list
data.frame(table(sample_data(sample.Hymenoptera)$host_genus) )
data.frame(table(sample_data(sample.Hymenoptera)$host_genus)) %>% arrange(desc(Freq))

sample_variables(sample.Hymenoptera)
sample.Halictinae <- subset_samples(sample.Hymenoptera, host_subfamily=="Halictinae")
table(sample_data(sample.Halictinae )$group_tree.1)

subfamily_bee <- names(table(sample_data(sample.Hymenoptera)$host_subfamily))
# "Apinae"     "Halictinae"
tribe_bee <- names(table(sample_data(sample.Hymenoptera)$host_tribe))
# "Apini"      "Bombini"    "Euglossini" "Halictinae" "Meliponini"

## Compare both datasets (if there is a larger taxa discrepancy)
data.frame(sort(taxa_sums(sample.Lepidoptera), decreasing = T)) # Remove zero counts from dataset 
sample.Lepidoptera.condensed = prune_taxa(taxa_sums(sample.Lepidoptera )>100, sample.Lepidoptera)
data.frame(sort(taxa_sums(sample.Lepidoptera.condensed), decreasing = T)) 

data.frame(sort(taxa_sums(sample.species), decreasing = T)) 

# Create abundance tables
df1 <- data.frame(genus = as.character(tax_table(sample.species)[, "genus"]),
  abundance = taxa_sums(sample.species) )

df2 <- data.frame(genus = as.character(tax_table(sample.Lepidoptera.condensed)[, "genus"]),
  abundance = taxa_sums(sample.Lepidoptera.condensed) )

# Genera unique to each dataset
only_ps1 <- df1 |>
  dplyr::filter(!genus %in% df2$genus) |>
  dplyr::arrange(desc(abundance))

only_ps2 <- df2 |>
  dplyr::filter(!genus %in% df1$genus) |>
  dplyr::arrange(desc(abundance))

head(only_ps1, 20) # only Lep dataset
head(only_ps2, 10) # only Lep bee dataset
...
## Compare insect order differences Lep / Hym
sample.subset = subset_taxa(sample.bee.rel, family == "Lactobacillaceae" ) 
sample.subset = subset_taxa(sample.bee.rel, family == "Acetobacteraceae" )
sample.subset = subset_taxa(sample.bee.rel, family == "Orbaceae" )
sample.subset = subset_taxa(sample.bee.rel, family == "Selenomonadaceae" ) # 
sample.subset = subset_taxa(sample.bee.rel, family == "Moraxellaceae" | family == "Pseudomonadaceae"  ) #

my_group <- "genus" # bacterial genus, family etc.

sample.subset.grouped.order <- psmelt(sample.subset) %>%
  group_by(Sample, host_order, dplyr::across(all_of(my_group))) %>% 
  summarise(group_ab = sum(Abundance, na.rm = TRUE)) %>%  
  group_by(host_order, dplyr::across(all_of(my_group))) %>%         
  summarise(
    mean_abundance = mean(group_ab, na.rm = TRUE),
    sd_abundance   = sd(group_ab, na.rm = TRUE),
    .groups = "drop"   )

# plot rel ab. (per host_order) dynamically grouped by my_group (e.g. bacterial family) 
ggplot(sample.subset.grouped.order,
       aes(x = host_order, y = mean_abundance, fill = !!sym(my_group) )) +
  geom_bar(colour="black", stat="identity", linewidth=0.3) +
  theme_line2() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, face="italic")) +
  scale_y_continuous(limits = c(0, 1),labels = scales::label_percent(scale = 100, prefix = "", suffix = "")) + # y-axis 100%
  theme(legend.title=element_text(size=11), legend.text=element_text(size=10, face="italic")) + # legend text
  labs(x="",y="Rel Ab [%]") 

### Hym/Lep core stripchart    ----- 

# Butterfly core taxa
prevalence(sample.Lepidoptera.rel, detection = 5/100, sort = TRUE)[1:20] # 
prevalence(sample.Lepidoptera.rel, detection = 1/100, sort = TRUE)[1:20] # 
prevalence(sample.Lepidoptera.rel, detection = 1/500, sort = TRUE)[1:20] # 

lep.core.genus.high <- core_members(sample.Lepidoptera.rel, detection = 0.05, prevalence = 10/100)  # 11 taxa
lep.core.genus      <- core_members(sample.Lepidoptera.rel, detection = 0.01, prevalence = 20/100)  # 12 taxa
lep.core.genus.low  <- core_members(sample.Lepidoptera.rel, detection = 0.002, prevalence = 40/100) # 12 taxa

# Check taxa 
check_taxa <- subset_taxa(sample.Lepidoptera.rel, genus == "Floricoccus")
sort(data.frame(sum = sample_sums(check_taxa)*100,
                host_genus = sample_data(check_taxa)$host_genus), decreasing = FALSE)
sort(data.frame(sum = taxa_sums(check_taxa)), decreasing = FALSE)


check_taxa <- subset_taxa(sample.bee.ASV.Lepidoptera, genus == "Pectinatus")
check_taxa <- subset_taxa(sample.bee.ASV.Lepidoptera, family == "Pectobacteriaceae")
sort(data.frame(sum = sample_sums(check_taxa),
                host_genus = sample_data(check_taxa)$host_genus), decreasing = FALSE)
sort(data.frame(sum = taxa_sums(check_taxa)), decreasing = FALSE)
...
# Bee core taxa
prevalence(sample.Hymenoptera.rel, detection = 5/100, sort = TRUE)[1:20] # 
prevalence(sample.Hymenoptera.rel, detection = 1/100, sort = TRUE)[1:20] # 
prevalence(sample.Hymenoptera.rel, detection = 1/500, sort = TRUE)[1:20] # 

bee.core.genus.high <- core_members(sample.Hymenoptera.rel, detection = 0.05, prevalence = 10/100)  # 11 taxa
bee.core.genus      <- core_members(sample.Hymenoptera.rel, detection = 0.01, prevalence = 20/100)  # 15 taxa
bee.core.genus.low  <- core_members(sample.Hymenoptera.rel, detection = 0.002, prevalence = 40/100) # 17 taxa

bee.core_overlap <- list(high = bee.core.genus.high, med = bee.core.genus ,
                         low = bee.core.genus.low )

# Overlap of three core definitions
venn_bee.core_overlap <-  ggVennDiagram(bee.core_overlap, label = "count", label_alpha = 0, set_size = 3) +
  scale_fill_gradient(low = "white", high = "steelblue") +
  theme(legend.position = "none") +
  scale_x_continuous(expand = expansion(mult = 0.22)) +  # Make Venn smaller
  scale_y_continuous(expand = expansion(mult = 0.1))
...
# Shared core across orders 
core_overlap_order <- list(butterfly = lep.core.genus, wild_bees = bee.core.genus)

core_shared_order <- Reduce(intersect, core_overlap_order) # 6 taxa overlap from both insect orders

# Overlap of three core definitions
venn_core_overlap_order <-  ggVennDiagram(core_overlap_order, label = "count", label_alpha = 0, set_size = 3) +
  scale_fill_gradient(low = "white", high = "steelblue") +
  theme(legend.position = "none") +
  scale_x_continuous(expand = expansion(mult = 0.22)) +  # Make Venn smaller
  scale_y_continuous(expand = expansion(mult = 0.1))

# Combined genus core list Hym/Lep
combined_core <- union(lep.core.genus, bee.core.genus) 

combined_core.rel <- prune_taxa(combined_core, sample.bee.rel)
comb.core.melt <- psmelt(combined_core.rel)
comb.core.melt$host_order <- factor(comb.core.melt$host_order,
                                    levels = c("Lepidoptera", "Hymenoptera") )


# Stats kruskal.test core genera abundance per order 
core_stats_order  <- run_kruskal(comb.core.melt, "host_order")
data.frame(core_stats_order) # BH correction for adj p


# Make label for figure
core_stats_order_label <- core_stats_order %>%
  mutate(p_label = ifelse(p_adj < 0.001, "p < 0.001",
                          sprintf("p = %.3f", p_adj)), label = paste(p_label, signif),
         p_color = ifelse(p_adj < 0.05, highest_in, "ns"))


data.frame(core_stats_order_label)

# sort core genus names by their order ratio
comb.core_sorted_taxa <- comb.core.melt %>%
  group_by(genus, host_order) %>%
  summarise(total = sum(Abundance), .groups = "drop") %>% 
  pivot_wider(names_from = host_order, values_from = total, values_fill = 0) %>%
  mutate(Ratio = Hymenoptera / (Hymenoptera + Lepidoptera)) %>%
  arrange(Ratio) %>%
  pull(genus)

order_names <- names(table(sample_data(sample.bee.species)$host_order))
order_colorfix <- c("Lepidoptera" = "#E69F00",  "Hymenoptera" = "#A6D8F5") 

comb.core_stripchart.stats <- ggplot(comb.core.melt, aes(x = host_order, y = Abundance)) +
  geom_jitter(aes(color = host_order),width = 0.2, size = 3, alpha = 0.6) +  # points on top
  geom_violin() +
  stat_summary(fun = mean,  geom = "crossbar", width = 0.2,             
               color = "black",  linewidth = 0.5  ) +
  facet_wrap(~factor(genus, levels = comb.core_sorted_taxa), ncol = 6) +  # use sorted_taxa for facet order #facet_wrap(~genus)
  theme_grid() + theme(axis.text.x = element_text(size = 8), strip.text = element_text(face = "italic") ) +
  scale_colour_manual(values = order_colorfix) +
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "")) +
  labs(x = "", y = "rel. ab. [%]", color = "host order") +
  geom_text(data = core_stats_order_label,
            aes(x = 1.5, y = 0.95, label = label ), inherit.aes = FALSE, size = 3  ) 

# core stripchart separated groups sorted by 6 columns
group1 <- comb.core_sorted_taxa[1:6]   # only butterfly
group2 <- comb.core_sorted_taxa[7:12]  # shared bee butterfly
group3 <- comb.core_sorted_taxa[13:21] # only bee

group1_plot <- ggplot(comb.core.melt %>% filter(genus %in% group1), aes(x = host_order, y = Abundance)) +
  geom_jitter(aes(color = host_order), width = 0.2, size = 3, alpha = 0.6 ) +
  geom_violin() +
  stat_summary(fun = mean, geom = "crossbar", width = 0.2, color = "black", linewidth = 0.5 ) +
  facet_wrap(~ factor(genus, levels = group1), ncol = 6 ) +
  theme_grid() + scale_x_discrete(labels = c("Lepidoptera" = "Lep.", "Hymenoptera" = "Hym." )) + # rename axis label
  theme(axis.text.x = element_text(size = 8), axis.title.x = element_blank(), strip.text = element_text(face = "italic")) +
  scale_colour_manual(values = order_colorfix) +
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "" )) +
  labs(x = "", y = "rel. ab. [%]", color = "host order" ) +
  geom_text(data = core_stats_order_label %>% filter(genus %in% group1),
    aes(x = 1.5, y = 0.95, label = label, color = p_color), show.legend = FALSE, inherit.aes = FALSE, size = 3  )

group2_plot <- ggplot(comb.core.melt %>% filter(genus %in% group2), aes(x = host_order, y = Abundance)) +
  geom_jitter(aes(color = host_order), width = 0.2, size = 3, alpha = 0.6 ) +
  geom_violin() +
  stat_summary(fun = mean, geom = "crossbar", width = 0.2, color = "black", linewidth = 0.5 ) +
  facet_wrap(~ factor(genus, levels = group2), ncol = 6 ) +
  theme_grid() + scale_x_discrete(labels = c("Lepidoptera" = "Lep.", "Hymenoptera" = "Hym." )) + # rename axis label
  theme(axis.text.x = element_text(size = 8), axis.title.x = element_blank(), strip.text = element_text(face = "italic")) +
  scale_colour_manual(values =  order_colorfix) +
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "" )) +
  labs(x = "", y = "rel. ab. [%]", color = "host order" ) +
  geom_text(data = core_stats_order_label %>% filter(genus %in% group2),
            aes(x = 1.5, y = 0.95, label = label, color = p_color), show.legend = FALSE, inherit.aes = FALSE, size = 3  )

group3_plot <- ggplot(comb.core.melt %>% filter(genus %in% group3), aes(x = host_order, y = Abundance)) +
  geom_jitter(aes(color = host_order), width = 0.2, size = 3, alpha = 0.6 ) +
  geom_violin() +
  stat_summary(fun = mean, geom = "crossbar", width = 0.2, color = "black", linewidth = 0.5 ) +
  facet_wrap(~ factor(genus, levels = group3), ncol = 6 ) +
  theme_grid() + scale_x_discrete(labels = c("Lepidoptera" = "Lep.", "Hymenoptera" = "Hym." )) + # rename axis label
  theme(axis.text.x = element_text(size = 8), axis.title.x = element_blank(), strip.text = element_text(face = "italic")) +
  scale_colour_manual(values =  order_colorfix) +
  scale_y_continuous(labels = scales::label_percent(scale = 100, prefix = "", suffix = "" )) +
  labs(x = "", y = "rel. ab. [%]", color = "host order" ) +
  geom_text(data = core_stats_order_label %>% filter(genus %in% group3),
            aes(x = 1.5, y = 0.95, label = label, color = p_color), show.legend = FALSE, inherit.aes = FALSE, size = 3  )

comb.core_stripchart.combined <-
  group1_plot /
  group2_plot /
  group3_plot +
...
pdf(file.path(out_dir, "07_Hym_Lep_core_stripchart.pdf"), width=12, height=10)
venn_bee.core_overlap
venn_core_overlap_order
comb.core_stripchart.stats
comb.core_stripchart.combined
dev.off()
...
rm(group3_plot )
rm(comb.core.melt)
rm(comb.core_stripchart.stats)
rm(sample.bee.species)


### Hym/Lep Top order mirrored -----

# Take rel abundance dataset 
summary(sample_sums(sample.bee.rel))

# Group by inset order (for two sided mirror)
insect.order.frame <- psmelt(sample.bee.rel) %>%
  group_by(host_order, order, family, genus) %>%
  summarise(
    Abundance = mean(Abundance), .groups = "drop"  )

# check that both insect order sum up to 1
insect.order.frame %>%
  group_by(host_order) %>%
  summarise(total = sum(Abundance))

# Select top bacterial order
order_abundance <- insect.order.frame %>%
  group_by(order) %>%
  summarise(
...
# Check top 8 order
tail(order_abundance,20)

Top.bee.order <- order_abundance %>%
  slice_max(Abundance, n = 9) %>%
  pull(order)

# Remove as not relevant for Lepidoptera
Top.bee.order <- setdiff(Top.bee.order, c("Neisseriales"))
Top.bee.order 

# Filter top order from dataframe
insect.top.order.frame <- insect.order.frame %>%
  filter(order %in% Top.bee.order )

# check remaining percent per insect order (should be less than 1)
insect.top.order.frame %>%
  group_by(host_order) %>%
  summarise(total = sum(Abundance))
...
family_abundance <- insect.top.order.frame %>%
  #filter(host_order == "Lepidoptera") %>%
  group_by(family) %>%
  summarise(
    Abundance = sum(Abundance),
    .groups = "drop"
...
tail(family_abundance,20)

Top.bee.fam <- family_abundance %>%
  slice_max(Abundance, n = 15) %>%
  pull(family)

Top.bee.fam 

# filter top family
insect.top.fam.frame <- insect.top.order.frame %>%
  filter(family %in% Top.bee.fam  )

# check remaining percent per insect order (should be less than 1)
insect.top.fam.frame %>%
  group_by(host_order) %>%
  summarise(total = sum(Abundance))
...
bee.top.tornado <- insect.top.fam.frame %>%
  group_by(host_order, order, family, genus) %>%
  summarise(Abundance = sum(Abundance), .groups = "drop") %>%    
  mutate(Abundance_mirrored = ifelse(host_order == "Hymenoptera", Abundance, -Abundance)) %>%
  ungroup()

bee.top.tornado$host_order <- factor(bee.top.tornado$host_order, levels = c("Lepidoptera", "Hymenoptera") )
# Make sure 'order' in the tornado data is a factor with the correct level order
bee.top.tornado$order <- factor(bee.top.tornado$order, levels = Top.bee.order)

# Define color palette in Top.bee.order abundance order
pal <- colorRampPalette(brewer.pal(12, "Paired"))(23)
bee_order_palette <- setNames(pal[c(1,4,6,10,14,16,18,21)], Top.bee.order) #7 22,8

# Tornado top order / family min 1% rel abundance
Lep.Hym.order.fam.mirrored <- ggplot(bee.top.tornado , aes(family, Abundance_mirrored, fill= order)) +
  facet_grid(order~host_order, space = "free", scales = "free",switch = "y")+ # switch = "y" remove order names
  theme_grid()+
  geom_bar(stat="identity")+
  theme(strip.text.y.left = element_blank(),  # remove the order label
        axis.title.y = element_blank()) +    # remove "family" title
  scale_y_continuous(labels = function(x) scales::percent(abs(x))) + # show % as positive
  #scale_fill_manual(values = bee_order_palette) +
  coord_flip() + labs(y = "rel. ab. [%]")   

# Tornado top order / family min 1% rel abundance / division by genus
Lep.Hym.order.fam.mirrored.genus <- ggplot(bee.top.tornado, 
                                           aes(x = family, y = Abundance_mirrored, fill = order, group = genus)) +
  facet_grid(order ~ host_order, space = "free", scales = "free", switch = "y") +
  geom_bar(stat = "identity", colour = "black", linewidth = 0.3) +
  theme_grid() +
...
### Hym/Lep Group merged Top order
sample.bee.order.subfam.merged = merge_samples(sample.order, "host_subfamily")
sample.bee.order.subfam.merged.rel <- transform_sample_counts(sample.bee.order.subfam.merged, function(x) x/sum(x))

sample.bee.order.top <- subset_taxa(sample.bee.order.subfam.merged.rel, taxa_names(sample.bee.order.subfam.merged.rel)%in%Top.bee.order )
sample.bee.order.top.melt <- psmelt(sample.bee.order.top)
toporderCount = length(unique(sample.bee.order.top.melt$order))

remaining_percent_order_bee <- sum(sample_sums(sample.bee.order.top))*100/nsamples(sample.bee.order.top)
remaining_percent_order_bee # 85.56825

# order bacterial taxa by Top.order same as order.fam.country.mirrored.genus
sample.bee.order.top.melt$order <- factor(sample.bee.order.top.melt$order, levels = Top.bee.order) # order names by order

# Set Sample as an ordered factor according to subfamily_ordered 
sample.bee.order.top.melt$Sample <- factor(
  sample.bee.order.top.melt$Sample,
  levels = subfamily_ordered)

# reverse sample order for coord_flip
sample.bee.order.top.melt$Sample2 <- factor(sample.bee.order.top.melt$Sample, levels = rev(c(subfamily_ordered)))

top_order_genera.flip <- ggplot(sample.bee.order.top.melt,aes(x = Sample2, y=Abundance, fill = order))+
  geom_bar(#position="fill",
    colour="black", stat="identity", linewidth=0.3) + 
  scale_fill_manual(values = bee_order_palette)+
  theme_grid() + 
...
# Top 8 order abundance per host_subfamily
top_order_abundance_bee <- sample.bee.order.top.melt %>%
  group_by(Sample) %>%
  summarise(total_abundance = sum(Abundance)*100)

top_order_total_abundance_bee <- top_order_abundance_bee %>%
  summarise(
    mean_total_abundance = mean(total_abundance),
    sd_total_abundance = sd(total_abundance)   )

top_order_total_abundance_bee # 85.6
remaining_percent_order_bee   # 85.57

pdf(file.path(out_dir, "07_Hym_Lep_mirror.pdf"), width=12, height=6)
Lep.Hym.order.fam.mirrored
Lep.Hym.order.fam.mirrored.genus 
top_order_genera.flip
dev.off()

### Hym/Lep NMDS adonis -----
  
# Use sqrt transformation (similar to Hellinger transformation) to reduce NMDS stress level
sample.bee.sqrt <- transform_sample_counts(sample.bee.rel, sqrt)
set.seed(321)
bee.nmds.sqrt <- ordinate(sample.bee.sqrt, method="NMDS",distance = "bray", k=3, trymax=100)
bee.nmds.sqrt.stress <- round(bee.nmds.sqrt$stress, 3)
bee.nmds.sqrt.stress
# dimensions: 3 Stress:     0.166 # better 3 dimensions with sqrt transformation


nmds.sqrt.bee.all <- plot_ordination(sample.bee.rel,bee.nmds.sqrt, color="host_order", shape="country")+
  geom_point(size=4) + theme_grid() + stat_ellipse(aes(group = factor(host_order))) +
  annotate("text", x = Inf, y = -Inf, label = paste("Stress =", bee.nmds.sqrt.stress), 
           hjust = 1.1, vjust = -0.5, size = 4)


scores(bee.nmds.sqrt)$sites
rownames(scores(bee.nmds.sqrt)$sites) == row.names(sample_data(sample.bee.rel)) # check if true before cbind
bee.binder.sqrt <- cbind(scores(bee.nmds.sqrt)$sites,sample_data(sample.bee.rel))

bee.binder.sqrt$host_order <- factor(bee.binder.sqrt$host_order,
                                     levels = c("Lepidoptera", "Hymenoptera") )

lep.bee.binder.sqrt <- subset(bee.binder.sqrt, host_order == "Lepidoptera")
hym.bee.binder.sqrt <- subset(bee.binder.sqrt, host_order == "Hymenoptera")

Hym_colors <- c("Apinae" = "grey70", "Halictinae" = "grey50" ) 

# NMDS bee color
NMDS.bee.binder.labels.sqrt <- ggplot(bee.binder.sqrt, aes(x=NMDS1,y=NMDS2, color= host_order, shape = country)) + 
  #stat_ellipse(linewidth=1, alpha=0.8, aes(group = factor(host_order))) +
  #stat_ellipse(aes(group = factor(country)))+
  geom_point(data = hym.bee.binder.sqrt, size=3, aes(color=host_subfamily, shape = country), alpha = 0.6) +
  scale_colour_manual(name = "Hymenoptera", values = Hym_colors, guide = guide_legend(order = 2)     ) +
  ggnewscale::new_scale_colour() +
  geom_point(data = lep.bee.binder.sqrt, size=4, aes(color=host_subfamily), alpha = 0.8) +
  scale_colour_manual(name = "Lepidoptera", values = subfamily_colorfix, guide = guide_legend(order = 1) )  +
  theme_grid() + annotate("text", x = Inf, y = -Inf, label = paste("Stress =", bee.nmds.sqrt.stress), 
                          hjust = 1.1, vjust = -0.5, size = 3)


NMDS.bee.binder.twocolor <- ggplot(bee.binder.sqrt, aes(x=NMDS1,y=NMDS2, color= host_order)) + 
  stat_ellipse(linewidth=1, alpha=0.5, aes(group = factor(host_order))) +
  geom_point(size=3, aes(color=host_order, shape = country), alpha = 0.6) +
  scale_colour_manual(values = order_colorfix) +
  guides(color = guide_legend(order = 1,  override.aes = list(linetype = 0,  shape = 16,alpha = 1  )),
         shape = guide_legend(order = 2)  ) +
  labs(color = "host order") +
  theme_grid() + annotate("text", x = Inf, y = -Inf, label = paste("Stress =", bee.nmds.sqrt.stress), 
                          hjust = 1.1, vjust = -0.5, size = 4)

bee_tribe_color = brewer.pal(n = 12, "Set3")[1:length(tribe_bee)]
bee_tribe_colorfix <- setNames(bee_tribe_color, tribe_bee)

NMDS.bee.binder.tribe.color <- ggplot(bee.binder.sqrt, aes(x=NMDS1,y=NMDS2, color= host_order, shape = country)) + 
  geom_point(data = hym.bee.binder.sqrt, size=3, aes(color=host_tribe, shape = country), alpha = 0.6) +
  scale_colour_manual(name = "Hymenoptera",  values = bee_tribe_colorfix, guide = guide_legend(order = 2)     ) +
  facet_wrap(~host_order) +
  ggnewscale::new_scale_colour() +
  geom_point(data = lep.bee.binder.sqrt, size=4, aes(color=host_tribe), alpha = 0.8) +
  scale_colour_manual(name = "Lepidoptera", values = tribe_colorfix, guide = guide_legend(order = 1) )  +
  theme_grid() + annotate("text", x = Inf, y = -Inf, label = paste("Stress =", bee.nmds.sqrt.stress), 
                          hjust = 1.1, vjust = -0.5, size = 3)


nmds.sub <- subset(bee.binder.sqrt,
                   host_order %in% c("Hymenoptera") )

NMDS.binder.bee.only.subset <- ggplot(nmds.sub, aes(x=NMDS1,y=NMDS2))+
  #stat_ellipse(level = 0.9, linewidth = 1, aes(group = host_tribe, color = host_tribe)) +
  geom_point(size = 5, aes(color = host_tribe), alpha = 0.8) +
  #facet_wrap(~host_tribe) +
  scale_colour_manual(values = bee_tribe_colorfix) +
  theme_grid()  #+ geom_label(aes(label = group_tree), size = 3)

nmds.halic <- subset(bee.binder.sqrt,
                     host_subfamily %in% c("Halictinae") )

NMDS.binder.halic <- ggplot(nmds.halic, aes(x=NMDS1,y=NMDS2))+
  #stat_ellipse(level = 0.9, linewidth = 1, aes(group = host_tribe, color = host_genus)) +
  geom_point(size = 5, aes(shape = country, color = host_genus), alpha = 0.8) +
  facet_wrap(~host_tribe) +
  #scale_colour_manual(values = tribe_colorfix) +
  theme_grid()  #+ geom_label(aes(label = bc_genus), size = 3)



### Hym/Lep adonis 
adonis.bee  <- adonis2(phyloseq::distance(sample.bee.sqrt, method = "bray")~ host_order    , data=bee.binder.sqrt,  permutations = 999, by = "margin")
adonis.bee 


pdf(file.path(out_dir, "07_Hym_Lep_NMDS_K3.pdf"), width=8, height=8)
NMDS.bee.binder.labels.sqrt
NMDS.bee.binder.twocolor
NMDS.bee.binder.tribe.color 
NMDS.binder.bee.only.subset
NMDS.binder.halic
dev.off()
...
### Hym/Lep ggtree new bee core label ----

bee_core_taxa <- taxa_data[rownames(taxa_data) %in% bee.core.genus | taxa_data[,"genus"] %in% bee.core.genus , ]

bee_core_tips <- data.frame(genus = bee_core_taxa[,"genus"],  # Extract bee core genus names
                            label = rownames(bee_core_taxa) ) # Extract ASV/tip labels


fan.core.bee.core <- fan.core.point  + 
  geom_tiplab(size = 6, align = TRUE, offset = 0.001, # nudge_y = -0.3, # offset = 0.14
              aes(label = ifelse(label %in% bee_core_tips$label, "\u2022", "")), # * or bold asteriks ✱ \u2022
              linetype = NA, # NA dotted
              family = "mono")   # Only show * for matching tips  

 

fan.core.annotation.bee.core <- fan.core.bee.core + annotate("text",  x = 0,  y = 0, 
                                                             label = "rel. ab. [%]", size = 2.8, angle = 0, hjust = -3.8,  vjust = 2.5) +
  theme(legend.key.size = unit(0.6, "lines"),  # smaller legend (usually 1 line)
        legend.position = "inside",
        legend.position.inside = c(0.82, 0.28), # place legend closer to plot
        legend.justification = c(0, 0.5),     # Right-center of the legend box
        plot.margin = margin(-40, 2, -40, -20) )   # increase right margin for legend
# figure border highlight for diagnostics
# theme(panel.border = element_rect(color = "blue", fill = NA, linewidth = 1)) 
...
# Get prevalence data from bee dataset
tree_genus_names <- taxa_data$genus # get genus names from tree
# subset bee dataset to fit taxa in tree
sample.Hymenoptera.treefit <- prune_taxa(tax_table(sample.Hymenoptera)[, "genus"] %in% tree_genus_names, sample.Hymenoptera )

# Separate Apinae and Halictidae for individual prevalence 
Apinae.treefit <- subset_samples(sample.Hymenoptera.treefit, host_subfamily=="Apinae")
Halictinae.treefit <- subset_samples(sample.Hymenoptera.treefit, host_subfamily=="Halictinae")

Apinae_prev <- data.frame(
  genus = as.character(tax_table(Apinae.treefit)[, "genus"]),
  prevalence = apply(otu_table(Apinae.treefit), 1, function(x) sum(x > prev_cutoff)) )
Apinae_prev$rel_prev <- Apinae_prev$prevalence / ncol(otu_table(Apinae.treefit))


Halictinae_prev <- data.frame(
  genus = as.character(tax_table(Halictinae.treefit)[, "genus"]),
  prevalence = apply(otu_table(Halictinae.treefit), 1, function(x) sum(x > prev_cutoff)) )
Halictinae_prev$rel_prev <- Halictinae_prev$prevalence / ncol(otu_table(Halictinae.treefit))

head(square_taxa_data)
# Add bee prevalence to each ASV tip label for tree
square_taxa_data$Apinae <- Apinae_prev$rel_prev[
  match(square_taxa_data$tree_genus, Apinae_prev$genus) ]

square_taxa_data$Halictinae <- Halictinae_prev$rel_prev[
  match(square_taxa_data$tree_genus, Halictinae_prev$genus) ]

# Genera absent from bees get 0 
square_taxa_data$Apinae[is.na(square_taxa_data$Apinae)] <- 0
square_taxa_data$Halictinae[is.na(square_taxa_data$Halictinae)] <- 0

head(square_taxa_data)

# bee prev dataframe for tree
bee_prev.df <- square_taxa_data[, c("label", "Apinae", "Halictinae")]
rownames(bee_prev.df) <- bee_prev.df$label
bee_prev.df$label <- NULL

# square core with bee subfamilies

square.prev.bee <- gheatmap(square.core, bee_prev.df, 
                 width = 0.08,
                 offset = 0.084, # 0.035, 0.06
                 colnames_position = "top",
                 colnames_angle =60, font.size = 3,
...
square.prev.bee.all <- gheatmap(square.prev.bee, combined_rel_prev.df, 
                  width = 0.2,
                  offset = 0.13, # 0.035, 0.06
                  colnames_position = "top",
                  colnames_angle =60, font.size = 3,
                  hjust = 0,
                  colnames_offset_y = 0.5) +
...
square.core.bee.bar <- square.prev.bee.all + new_scale_fill() +
  geom_fruit(data=taxonomy_df, geom=geom_bar, 
             mapping=aes(y=label, x=rel_ab, fill=phylum), 
             pwidth=0.5, # 0.5 50% space for geom bar
             offset = 0.53, # 0.45
...
square.core.bee.label <- square.core.bee.bar  + geom_tiplab(size = 2.5, align = T, offset = 0.23, #nudge_y = 0.3 , # 0.19
                                               aes(label = ifelse(label %in% core_tips$label, core_tips$genus_name[match(label, core_tips$label)], "")), linetype = NA)  # Only show core genera

square.core.bee <- square.core.bee.label + geom_tiplab(size = 5, align = TRUE, offset = 0.08, #nudge_y = -0.4,
                                              aes(label = ifelse(label %in% bee_core_tips$label, "\u2022", "")),
                                              linetype = NA, family = "mono")   # Only show * for matching tips

square.core.bee.point <- square.core.bee + geom_point(aes(color = taxonomy_df$phylum, x = x + 0.005),
                                             data = ~ .x[.x$isTip, ],  
                                             size = 2, show.legend = FALSE) + scale_color_manual(values = color_phyla)

square.core.bee.annotation <-  square.core.bee.point +   geom_treescale(fontsize=3, linesize=0.1, x=0, y=-2) +
theme(legend.key.size = unit(0.8, "lines"), 
      legend.position = "inside",
      legend.position.inside=c(0.1, 0.7),
      plot.margin = margin(50, 0, 10, 5)  )   
...
# Fig 1 arrange with patchwork 
fig1.arrange_revision <- (empty.plot)+ 
  (qPCR_plot_host.revision + theme(legend.position = "empty") ) +
  (NMDS.bee.binder.labels.sqrt+ theme(legend.position = "right") ) +
  plot_layout(ncol = 3, widths = c(1.2,0.9,1)) &  
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(face = "bold", size = 14, hjust = 0),
        plot.tag.position = c(0.01, 0.98),
        plot.margin = margin(3, 3, 3, 3),
        legend.key.size = unit(0.8, "lines") )      # compact legend
...
betadisp.subfamily.ordered_noleg <- betadisp.subfamily.ordered  + theme(legend.position = "none")
PCoA.host.overview.tribe_theme <- PCoA.host.overview.tribe + theme(legend.key.size = unit(0.9, "lines") )

fig2.betadisp.tribe.wrap.REV <- ggarrange( betadisp.subfamily.ordered_noleg, PCoA.host.overview.tribe_theme , marginalR2.plot,   labels = c('a', 'b', 'c' ),
                                       common.legend = F, ncol = 3,   nrow = 1, align = "h", widths = c(1 ,2, 0.4) )


# Fig 3 arrange with patchwork
fig3.gen.core.abundance.REV <- (order.core + theme(legend.position = "right"))+ 
  (top_order_genera.flip + theme(legend.position = "none") ) +
  (Lep.Hym.order.fam.mirrored.genus  #+ theme(legend.position = "none")
   ) +
...
s1.plot_alpha.country.rev <- 
  (qPCR_plot_host.genus.sort + theme(legend.position = "none")) +
  (qPCR_plot_country ) +
  (alpha.shannon.tribe.sort + theme(legend.position = "none"))+ 
  (alpha.subfamily.country  + theme(legend.position = "none") ) +
  plot_layout(nrow = 2, ncol = 2, widths = c(2, 1.2), guides = "collect") &  # , guides = "collect" #  c(2, 1, 2)
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(face = "bold", size = 14, hjust = 0),
        plot.tag.position = c(0.01, 0.98),
        plot.margin = margin(3, 3, 3, 3),
...
s2.plot_alpha.temp <- (
  (sample.shannon.elevation.Peru  + theme(legend.position = "none")) + 
    (shannon.kw.Germany.noAglais2 + theme(legend.position = "none") ) +
    (temp.16Scopy + theme(legend.position = "none"))
)/( 
  (temp.richness.noAglais2 + theme(legend.position = "none")) +
    (temp.shannon.noAglais2 + theme(legend.position = "none") ) +
    (temp.InvSimpson.noAglais2 + theme(legend.position = "right")  )
)+
...
betadisp.genus_theme <-  betadisp.genus + theme(plot.tag = element_text(face = "bold", size = 14, hjust = 0), legend.key.size = unit(0.9, "lines") )

s3.betadisp.tribe.wrap2.rev <- ggarrange( PCoA.genus.wrap.tribe_noleg, betadisp.genus_theme,  labels = c('a', 'b'),
                                     common.legend = F, ncol = 2,   nrow = 1, align = "h", widths = c(1, 0.7) ) # , legend = "right"  c(1, 0.8)
...
pD <- noncore.genus.abundance.samplecolor + 
  theme(
...
# Supplement S5 sample core  non-core abundance
sample.core.ncore_col5 <- (sample.core.filter.abundance.grid5 | noncore.filter.grid5 ) +
  plot_annotation(tag_levels = "a") &
  theme(
    plot.tag = element_text(face = "bold", size = 14, hjust = 0),
...
# supplement S10 Endosymbiont
s10.endosymbiont.arrange <- ggarrange(endo.bar3, endo.bar2,    labels = c('a', 'b'),
          common.legend = T, legend = "right", ncol = 2,   nrow = 1, align = "h", widths = c(1,2) )


# supplement S11 
s11.stripchart.arrange <- ggarrange( core.genus.boxplot , core_stripchart.rebuild, labels = c('a', 'b'),
...
ggsave(file.path(out_dir, "Fig1.png"), plot = fig1.arrange_revision, width = 30, height = 9, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "Fig1.pdf"), plot = fig1.arrange_revision, width = 30, height = 9, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "Fig2.png"), plot = fig2.betadisp.tribe.wrap.REV , width = 30, height = 9, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "Fig2.pdf"), plot = fig2.betadisp.tribe.wrap.REV , width = 30, height = 9, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "Fig3.png"), plot = fig3.gen.core.abundance.REV  , width = 30, height = 10, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "Fig3.pdf"), plot = fig3.gen.core.abundance.REV  , width = 30, height = 10, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "Fig4.png"), plot = fan.core.annotation.bee.core, width = 16, height = 16, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "Fig4.pdf"), plot = fan.core.annotation.bee.core, width = 16, height = 16, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "Fig5.png"), plot = fig5.net.core.comp.rev, width = 32, height = 11, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "Fig5.pdf"), plot = fig5.net.core.comp.rev, width = 32, height = 11, units = "cm", dpi = 300)

### ggsave Supplemental Figures -----

ggsave(file.path(out_dir, "FigS1_alpha.country.png"), plot = s1.plot_alpha.country.rev , width = 26, height = 22, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS1_alpha.country.pdf"), plot = s1.plot_alpha.country.rev , width = 26, height = 22, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS2_alphaelevation.png"), plot = s2.plot_alpha.temp  , width = 30, height = 18, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS2_alphaelevation.pdf"), plot = s2.plot_alpha.temp  , width = 30, height = 18, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS3_beta_genus.png"), plot = s3.betadisp.tribe.wrap2.rev , width = 28, height = 14, units = "cm", dpi = 300) # 30 13 # 28 16 # 26 15
ggsave(file.path(out_dir, "FigS3_beta_genus.pdf"), plot = s3.betadisp.tribe.wrap2.rev , width = 28, height = 14, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS4_corevenn_alternative.png"), plot = s4.patch_venn_core_noncore_plot , width = 32, height = 22, units = "cm", dpi = 300) # width = 34, height = 22,
ggsave(file.path(out_dir, "FigS4_corevenn_alternative.pdf"), plot = s4.patch_venn_core_noncore_plot , width = 32, height = 22, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS5_sample_core_noncore.png"), plot = sample.core.ncore_col5 , width = 30, height = 34, units = "cm", dpi = 300) # width = 34, height = 22,
ggsave(file.path(out_dir, "FigS5_sample_core_noncore.pdf"), plot = sample.core.ncore_col5 , width = 30, height = 34, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS6_core_square.png"), plot = square.core.bee.annotation , width = 24, height = 22, units = "cm", dpi = 300) # width = 26, height = 24
ggsave(file.path(out_dir, "FigS6_core_square.pdf"), plot = square.core.bee.annotation , width = 24, height = 22, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS7_Hym_Lep_core_stripchart.png"), plot = comb.core_stripchart.combined , width = 24, height = 20, units = "cm", dpi = 300) # width = 34, height = 22,
ggsave(file.path(out_dir, "FigS7_Hym_Lep_core_stripchart.pdf"), plot = comb.core_stripchart.combined , width = 24, height = 20, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS8_country_core.png"), plot = core_stripchart.country.stats, width = 24, height = 15, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS8_country_core.pdf"), plot = core_stripchart.country.stats, width = 24, height = 15, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS9_bubble.country.png"), plot = ASV.bubble.core.country , width = 24, height = 28, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS9_bubble.country.pdf"), plot = ASV.bubble.core.country , width = 24, height = 28, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS10_endo.png"), plot = s10.endosymbiont.arrange , width = 24, height = 12, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS10_endo.pdf"), plot = s10.endosymbiont.arrange , width = 24, height = 12, units = "cm", dpi = 300)

ggsave(file.path(out_dir, "FigS11_corestrip.png"), plot = s11.stripchart.arrange , width = 24, height = 12, units = "cm", dpi = 300)
ggsave(file.path(out_dir, "FigS11_corestrip.pdf"), plot = s11.stripchart.arrange , width = 24, height = 12, units = "cm", dpi = 300)
...
runtime <- difftime(pipeline_end, pipeline_start, units = "mins")

message("Pipeline finished at ", pipeline_end)
message("Total runtime: ", round(as.numeric(runtime), 2), " minutes")
